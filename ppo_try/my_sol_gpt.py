...
from given_info.utils import load_problem_data, save_solution
from given_info.evaluation import evaluation_function, check_datacenter_slots_size_constraint, update_check_lifespan

# Configure the loguru logger
logger.add("data_center_env.log", format="{time} {level} {message}", level="WARNING", rotation="1000 MB", compression="zip")
...
class DataCenterEnv(gym.Env):
    def __init__(self):
        ...
        # Action space: [Buy, Hold, Move, Dismiss] for each server generation
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(self._calculate_observation_space_shape(),),
                                            dtype=np.float32)
    ...
    def _apply_action(self, action):
        ...
        for idx, act in enumerate(action):
            ...
            if not self._is_server_available(server_generation):
                logger.warning("Attempted to act on server {} not yet available at time step {}", server_generation,
        ...
        return (10 if valid_action else -5000), valid_action

    def _is_server_available(self, server_generation):
        release_time = eval(
            self.servers[self.servers['server_generation'] == server_generation]['release_time'].values[0])
        return self.current_time_step >= min(release_time) and self.current_time_step <= max(release_time)
    ...
    def _buy_server(self, server_generation):
        ...
        if not available_servers.empty:
            ...
            logger.info("Bought server: {} in datacenter: {}", new_server['server_generation'].values[0],
                        new_server['datacenter_id'].values[0])
            return True
    ...
    def _generate_solution_json(self):
        ...
        solution = solution.merge(self.datacenters, on='datacenter_id',
                                  how='left')  # Merge to ensure slots_capacity is included
        return solution
...
model.save("./output/ppo_datacenter_model")

logger.critical("Model trained and saved successfully.")
